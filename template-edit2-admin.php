...
$MOD = $_GET['MODALITY'];
include ("session.php");
$sql = "SELECT ID, NAME, XRAY_TYPE_CODE,BODY_PART, USER_ID from xray_report_template WHERE  XRAY_TYPE_CODE = '$MOD' ";
//$sql ="select XRAY_CODE, DESCRIPTION from xray_code where XRAY_TYPE_CODE = '$MOD'";
$result = mysqli_query($dbconnect, $sql);
echo "<table>";
while($row = mysqli_fetch_array($result))
	{
		echo "<tr><td>User ID: ".$row['USER_ID']."</td><td>".$row['NAME']."</td><td>".$row['XRAY_TYPE_CODE']."</td><td> - ".$row['BODY_PART']."</td>";
